...
public class minLengthOfPermutations {  
    ...
    public static void main(String[] args) {
        ...
        for (int t = 0; t < T; t++) {
            ...
            // Input the elements of the first array.array
            int[] A = new int[N];
            for (int i = 0; i < N; i++) {
                A[i] = scanner.nextInt();
            }

            // Input the elements of the second array.array
            int[] B = new int[N];
            for (int i = 0; i < N; i++) {
                B[i] = scanner.nextInt();
            }
    ...
    private static int findMinSubarrayLength(int N, int[] A, int[] B) {
        ...
        // Count occurrences of elements in array.array A
        for (int num : A) {
        ...
        // Iterate through array.array B
        while (end < N) {
            int currentNum = B[end];

            // Update countMap and diffCount based on the current element in array.array B
            ...
            if (end - start + 1 == N) {
                return 0; // Full array.array is a permutation, no need to sort
